...
function getMonthlyData(subindices: SubDetalhe[], ano: number, anoRef: number) {
  const previsto = MESES.map((m) =>
    subindices
      .filter((s) => getSubAno(s, anoRef) === ano)
      .reduce((a, s) => a + (s[m] ?? 0), 0)
  )
  ...
  const events: TimelineEvent[] = [
    {
      ...
      descricao: [
        ...
        contrato.num_os ? contrato.num_os : null,
      ].filter(Boolean).join(' · '),
    },
  ]
...
export default function ContratoVisaoGeralPage() {
  ...
  const anualData = useMemo(() =>
    contrato ? buildAnualData(contrato.subindices, contrato.ano_referencia) : [],
    [contrato])

  const timeline = useMemo(() =>
    contrato ? buildTimeline(contrato, historico) : [],
    [contrato, historico])

  if (loading) return (
    <div className="p-6 max-w-7xl mx-auto text-center py-20 text-gray-400">Carregando...</div>
  )
  if (!contrato) return (
    <div className="p-6 max-w-7xl mx-auto text-center py-20 text-gray-400">Contrato não encontrado.</div>
  )
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-900">
            {contrato.indice} — {contrato.cliente.nome}
          </h1>
          {contrato.descricao && (
        ...
        <div className="mt-3 bg-white border border-gray-200 rounded-lg p-3">
          ...
          <div className="h-2.5 bg-gray-100 rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${Math.min(100, percRealizado)}%` }}
            />
          </div>
        </div>
      </section>
      ...
      <section className="bg-white border border-gray-200 rounded-lg p-4">
        ...
        {anoSel !== null && chartData ? (
          ...
          <ContratoFaturamentoChart
            modo="anual"
            previsto={anualData.map((d) => d.previsto)}
            faturado={anualData.map((d) => d.faturado)}
            labels={anualData.map((d) => String(d.ano))}
          />
        )}
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-3">
          ...
          {anualData.filter((a) => a.previsto === 0 && a.faturado === 0).length === 0 && anualData.length > 1 && (
      ...
      <section className="bg-white border border-gray-200 rounded-lg p-4">
        <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4 flex items-center gap-2">
          <span>∿</span> Histórico do Contrato
        </h2>
        {timeline.length === 0 ? (
          ...
          {contrato.subindices.map((s) => {
            ...
            return (
              <tr key={s.id} className="hover:bg-gray-50">
                ...
                <td className={tdCls}>
                  <span className={saldo > 0 ? 'text-orange-600' : 'text-green-600'}>{formatCurrency(Math.abs(saldo))}</span>
                </td>
                <td className={tdCls}>
...
function TimelineItem({ event }: { event: TimelineEvent }) {
  const dotColor = {
    NF:       'bg-orange-400',
    NF_INATIVA: 'bg-gray-300',
    CONTRATO: 'bg-green-primary',
    EDICAO:   'bg-blue-400',
  }[event.tipo]
