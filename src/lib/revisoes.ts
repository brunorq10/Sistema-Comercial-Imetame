// ─────────────────────────────────────────────────────────────────────────────
// Aplicação de uma revisão de solicitação. Chamada APENAS após o orçamentista
// aprovar a revisão pendente (fluxo em /api/solicitacoes/revisoes-pendentes).
// Lógica movida do antigo POST /api/solicitacoes/[id]/nova-revisao.
// ─────────────────────────────────────────────────────────────────────────────
...
export const revisaoPayloadSchema = z.object({
...
export type RevisaoPayload = z.infer<typeof revisaoPayloadSchema>

export type AplicarRevisaoResult =
  | { ok: true; novaRevisao: number }
  | { ok: false; error: string; status: number }

export async function aplicarRevisao(solicitacaoId: number, d: RevisaoPayload, userId: number): Promise<AplicarRevisaoResult> {
  ...
  if (!sol || sol.cancelled_at) return { ok: false, error: 'Solicitação não encontrada', status: 404 }
  if (sol.as_sold) {
    return { ok: false, error: 'Esta solicitação já possui uma revisão As Sold. Não é possível criar novas revisões.', status: 409 }
  }
  ...
  const pendenteCount = await prisma.propostaComercial.count({
    where: { solicitacao_id: solicitacaoId, resultado: 'AGUARDANDO' },
  })
  ...
  const updateResult = await prisma.solicitacao.updateMany({
    where: { id: solicitacaoId, revisao_esperada: sol.revisao_esperada },
    data: {
  ...
  if (updateResult.count === 0) {
    return { ok: false, error: 'Revisão já foi criada. Recarregue a página e tente novamente.', status: 409 }
  }
  ...
  if (pendenteCount > 0 && sol.orcamentista_id) {
    createNotificacao(
      ...
      `A revisão ${revLabelNova} foi aberta. ${pendenteCount} proposta(s) pendente(s) foram encerradas automaticamente.`,
      '/orcamentos/painel',
    )
  }
